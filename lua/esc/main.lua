...
esc.openMenu = function()
	...
	local serverList = addSection(body, 'НАШИ СЕРВЕРА')
	local function addServerButton(name, ip)
		...
		end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]'))
	end
	addServerButton('SERVER #1', 'steam://connect/109.171.53.136:27030')
	addServerButton('SERVER #2', 'steam://connect/109.171.53.136:27025')
	addServerButton('SERVER #3', 'steam://connect/109.171.53.136:27035')

	local webList = addSection(body, 'ОСНОВНЫЕ ССЫЛКИ')
	addAltText(addButton(webList, 'DISCORD', function()
		gui.OpenURL('https://discord.gg/efBzECNrxY')
	end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]')
	addAltText(addButton(webList, 'VK GROUP', function()
		gui.OpenURL('https://vk.com/unli_group')
	end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]')
	addAltText(addButton(webList, 'CONTENT COLLECTIONS', function()
		gui.OpenURL('https://steamcommunity.com/id/cumoll/myworkshopfiles/?section=collections&appid=4000')
	end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]')
		addAltText(addButton(webList, 'ПРАВИЛА СЕРВЕРА', function()
		gui.OpenURL('http://xenon-project.nex-squad.ru/docs.unli.team/rules.html')
	end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]')
	addAltText(addButton(webList, 'НАШ САЙТ', function()
		gui.OpenURL('http://xenon-project.nex-squad.ru/unli.team/')
	end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]')
	addAltText(addButton(webList, 'ДОКУМЕНТАЦИЯ', function()
		gui.OpenURL('http://xenon-project.nex-squad.ru/docs.unli.team/')
	end), '[НАЖМИТЕ, ЧТОБ ПРИСОЕДИНИТЬСЯ]')
	addAltText(addButton(webList, 'О НАС', function()
		gui.OpenURL('http://xenon-project.nex-squad.ru/unli.team/about.html')
	end), '[НАЖМИТЕ, ЧТОБ ЗАЙТИ]')

	addButton(body, 'ОТКЛЮЧИТСЯ', function() LocalPlayer():ConCommand('disconnect\n') end):Dock(BOTTOM)
	addButton(body, 'ОСНОВНОЕ МЕНЮ', function() gui.ActivateGameUI() panel:Remove() end):Dock(BOTTOM)
end
